...
std::vector<trace_command> trace_parser::parse_commandlist_file() {
  ...
  while (!ifs.eof()) {
    ...
    if (line.empty())
      ...
      trace_command command;
      command.command_string = line;
      command.m_type = command_type::cuda_malloc;
      commandlist.push_back(command);
    } else if (line.substr(0, 13) == "cudaHostAlloc") {
      trace_command command;
      command.command_string = line;
      command.m_type = command_type::cuda_host_alloc;
      commandlist.push_back(command);
    }
...
void trace_parser::parse_alloc_info(const std::string &alloc_command,
                                    unsigned long long &address, 
                                    unsigned long long &count) {
  std::vector<std::string> params;
  split(alloc_command, params, ',');
