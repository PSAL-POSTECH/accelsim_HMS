...
int main(int argc, const char **argv) {
  ...
  for (unsigned i = 0; i < commandlist.size(); ++i) {
    ...
    if (commandlist[i].m_type == command_type::cpu_gpu_mem_copy) {
      ...
      std::cout << "ignoring memcpy command : " << commandlist[i].command_string << std::endl;
      continue;
    ...
    } else if (commandlist[i].m_type == command_type::cuda_malloc ||
               commandlist[i].m_type == command_type::cuda_host_alloc) {
      unsigned long long addre, Bcount;
      tracer.parse_alloc_info(commandlist[i].command_string, addre, Bcount);
      if (commandlist[i].m_type == command_type::cuda_malloc)
        std::cout << "launching cudamalloc command : " << commandlist[i].command_string << std::endl;
      else
        std::cout << "launching cudaHostAlloc command : " << commandlist[i].command_string << std::endl;

      m_gpgpu_sim->set_addr_range(addre, Bcount);

      // align start address to the multiple of the page size
      //unsigned long long remain = addre % m_gpgpu_sim->get_config().page_size;
      unsigned long long remain = addre % MIN_PREFETCH_SIZE;
      addre -= remain;
      //assert(addre % m_gpgpu_sim->get_config().page_size == 0);
      assert(addre % MIN_PREFETCH_SIZE == 0);

      // update bcount according to the aligned start address
      Bcount += remain;

      unsigned long long num_large_pages = (size_t)(Bcount / MAX_PREFETCH_SIZE);
      unsigned long long remainder = Bcount - (num_large_pages * MAX_PREFETCH_SIZE);
      unsigned long long corrected_remainder;
      ...
      Bcount = (num_large_pages * MAX_PREFETCH_SIZE) + corrected_remainder;

      // make pages managed pages
      m_gpgpu_sim->set_pages_managed(addre, Bcount);
      unsigned long long tempGPUPtr = addre;
      for (unsigned long long cur_size = 0; cur_size < Bcount; ) {
        if ((Bcount - cur_size) < MAX_PREFETCH_SIZE) {
          m_gpgpu_sim->getGmmu()->initialize_large_page(tempGPUPtr, Bcount - cur_size);
          break;
        } else {
          m_gpgpu_sim->getGmmu()->initialize_large_page(tempGPUPtr, MAX_PREFETCH_SIZE);
          cur_size += MAX_PREFETCH_SIZE;
          tempGPUPtr += MAX_PREFETCH_SIZE;
        }
      }
    } else {
      assert(false && "undefined command");
    }
...
gpgpu_sim *gpgpu_trace_sim_init_perf_model(int argc, const char *argv[],
  ...
  m_gpgpu_context->the_gpgpusim->g_the_gpu_config->convert_byte_string();
